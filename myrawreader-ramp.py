...
Info = """
     ...
     * Detector fields: eventlist = {4: 'TrgRamp', 5: 'Reco', 27: 'CLK', 26: 'TimeBase', 25: 'TimeBaseUnsync'}; lanestatuslist = {3: 'F', 2: 'E', 1: 'W', 0: 'MissingData'}
       Strings in eventlist can be used to filter the pages 
...
def readRDH(index):
    ...
    if index == 1:
        ...
        if str(RDHstopbit) == '1':
            if LASTTDTCOUNTER != TDTCOUNTER:
                print(str(int(RDHorbit,16))+';'+str(TDTCOUNTER))
                VECCX.append(int(RDHorbit,16))
                VECCY.append(int(TDTCOUNTER))
            LASTTDTCOUNTER = TDTCOUNTER
            if (TDTCOUNTER > 1000):
                Exit()
            TDTCOUNTER = 0
